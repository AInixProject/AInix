...
def test_example():
    ...
    f = io.StringIO(test_string)
    mock_example_context = MagicMock()
    load_yaml(f, example_context=mock_example_context)
    mock_example_context.add_many_to_many_default_weight.assert_called_once_with(
        ["Hello"], ["Bonjour"],
        examplecontext.TypePair(examplecontext.DEFAULT_X_TYPE, "fooType"))
